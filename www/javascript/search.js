...
export async function initSearch() {
  ...
  searchInput.addEventListener("input", function () {
        ...
        stationGroups[gid] = {
          name:
            lang === "ja"
              ...
              : s.station_name_en || s.station_name_jp || "Unknown",
          stations: [],
        };
      }
    ...
    matchedLines.forEach(([lineId, lineData]) => {
      const lineName =
        lang === "ja"
          ? lineData.name_jp || lineData.name_en
          : lineData.name_en || lineData.name_jp;
      html += `
                <div class="search-result flex items-center gap-3 px-5 py-3 cursor-pointer hover:bg-gray-50 border-b-[2px] border-black last:border-b-0"
                    ...
                    <span class="text-xs font-black ">${lineName}</span>
                </div>
            `;
    });
      ...
      if (group.stations.length === 1) {
        ...
        html += `
                    <div class="search-result flex items-center gap-3 px-5 py-3 cursor-pointer hover:bg-gray-50 border-b-[2px] border-black last:border-b-0"
                        ...
                        <span class="ml-auto text-[10px] font-black  text-gray-400">${lineName}</span>
                    </div>
                `;
      } else {
          ...
          .map((s) => {
            ...
            return `
                        <div class="search-result flex items-center gap-2 px-5 py-2 cursor-pointer hover:bg-gray-50"
                            ...
                            <span class="text-[10px] font-black ">${lineName}</span>
                        </div>
                    `;
          })
          .join("");
    ...
    searchResults.querySelectorAll(".search-result").forEach((item) => {
      item.addEventListener("click", async () => {
        ...
        if (type === "station") {
          playOkSound();
          const cached = window.allStations?.find(
            (s) => String(s.station_id || s.id) === String(id),
          );
          if (cached) {
          ...
          const lineStations = window.allStations?.filter(
