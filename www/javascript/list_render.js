    ...
    if (!state.currentPrefId && !state.currentCompId) {
        ...
        if (state.currentCompId) {
            targetLineIds = targetLineIds.filter(id => {
                ...
                return line && String(line.company_id || line.company_cd) === String(state.currentCompId);
            });
        }
    }
...
export function renderNextChunk() {
    const chunk = state.currentFilteredLines.slice(state.renderIndex, state.renderIndex + RENDER_CHUNK_SIZE);
