...
export async function showLineDetail(lineId) {
  ...
  selectors.detailLineName.onclick = () => {
    ...
    const panToMid = () => {
      if (stations?.length) {
        ...
        window.map.panTo({
          lat: Number(mid.lat || mid.displayLat),
          lng: Number(mid.lon || mid.displayLon),
        });
    ...
    if (filterPill) {
      ...
      document.getElementById("active-filter-color").style.backgroundColor =
        line.color || "#000";
  ...
  selectors.detailStationsList.innerHTML = stations
    .map((s) => {
      const visited = isVisited(s.id) || userStamps[String(s.id)];
      const stampHtml = userStamps[String(s.id)]
        ? `<div class="mt-4 mb-2 cursor-pointer stamp-image-preview transition-transform active:scale-95 w-max relative z-10" data-station-id="${s.id}"><img src="${userStamps[String(s.id)]}" class="w-32 h-32 object-cover border-[4px] border-black rounded-[20px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] bg-white -rotate-2 pointer-events-none"></div>`
        : "";
      const stationName =
        lang === "ja"
...
<span class="station-name-click cursor-pointer hover:text-gray-500 transition-colors text-xl font-black  tracking-tight pt-2" data-station-id="${s.id}" data-lat="${s.lat || s.displayLat}" data-lon="${s.lon || s.displayLon}">${escapeHtml(stationName)}</span>                <button class="add-stamp-btn bg-white border-[3px] border-black px-4 py-2 rounded-xl text-[10px] font-black  shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] transition-all w-max mt-1" data-station-id="${s.id}" data-station-name="${escapeHtml(stationName)}" data-line-color="${line.color || "#B2FF59"}">+ ${t("camera.newStamp")}</button>
                ${stampHtml}
            </div>
        </div>`;
    })
  ...
  tabStations.className =
    "flex-1 py-4 text-xs font-black  border-r-[4px] border-black bg-[#B2FF59]";
  tabModels.className =
    "flex-1 py-4 text-xs font-black  bg-gray-100 text-gray-400";
  ...
  if (selectors.detailModelsList)
    selectors.detailModelsList.classList.add("hidden");
  ...
  tabModels.onclick = () => {
    playSlideSound();
    tabModels.className = "flex-1 py-4 text-xs font-black  bg-[#B2FF59]";
    tabStations.className =
      "flex-1 py-4 text-xs font-black  border-r-[4px] border-black bg-gray-100 text-gray-400";
    selectors.detailStationsList.classList.add("hidden");
    selectors.detailTrackLine.classList.add("hidden");
    if (selectors.detailModelsList)
      selectors.detailModelsList.classList.remove("hidden");
...
function renderModelsList(lineId, line, localizedLineName) {
  ...
  const modelsForLine = Object.entries(userModels || {}).filter(
    ([k, v]) => String(v.line_id) === String(lineId),
  );

  let html = `
        <div class="flex flex-col gap-4 w-full">
            <button class="add-model-btn bg-white border-[4px] border-black px-6 py-4 rounded-[20px] text-lg font-black  shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] transition-all w-full mb-4" data-line-id="${lineId}" data-line-name="${escapeHtml(localizedLineName)}" data-line-color="${line.color || "#B2FF59"}">+ ${t("camera.newModel")}</button>
  ...
  modelsForLine.forEach(([modelId, data]) => {
    ...
    html += `
            <div class="cursor-pointer model-image-preview transition-transform active:scale-95 flex flex-col items-center gap-2" data-model-id="${modelId}">
                ...
                <span class="text-[10px] font-black  tracking-tight text-center truncate w-full px-1">${escapeHtml(modelName)}</span>
            </div>
        `;
  });
...
export function initStampScanner() {
  ...
  selectors.detailStationsList.addEventListener("click", (e) => {
    ...
    if (nameClick) {
      ...
      const station = state.localStations.find(
      ...
      if (filterPill && line) {
        const lang = getLanguage();
        const localizedLineName =
          lang === "ja"
            ? line.name_jp || line.name_en
            : line.name_en || line.name_jp;
        document.getElementById("active-filter-name").innerText =
          localizedLineName;
        document.getElementById("active-filter-color").style.backgroundColor =
          line.color || "#000";
        filterPill.classList.remove("hidden");
      ...
      if (window.showTooltip && station) {
        ...
        const stationName =
          lang === "ja"
            ? station.station_name_jp || station.station_name_en
            : station.station_name_en || station.station_name_jp;
        const lineName =
          lang === "ja"
            ? line?.name_jp || line?.name_en
            : line?.name_en || line?.name_jp;
  ...
  els.cropCanvas.onmousedown = (e) =>
    handleCropInput(e, els.cropCanvas, "down");
  els.cropCanvas.ontouchstart = (e) =>
    handleCropInput(e, els.cropCanvas, "down");
  window.addEventListener("mousemove", (e) =>
    handleCropInput(e, els.cropCanvas, "move"),
  );
  window.addEventListener(
    "touchmove",
    (e) => handleCropInput(e, els.cropCanvas, "move"),
    { passive: false },
  );
  ...
  els.refineMask.addEventListener(
    "touchmove",
    (e) =>
      handleRefineDraw(
        e,
  ...
  window.addEventListener("mouseup", () =>
    handleRefineDraw(null, null, null, null, "stop", isFlipped),
  );
  ...
  document.getElementById("tool-brush").onclick = () => {
    playOkSound();
    currentTool = "brush";
    document.getElementById("tool-brush").classList.add("border-white");
    document.getElementById("tool-erase").classList.remove("border-white");
  };
  document.getElementById("tool-erase").onclick = () => {
    playOkSound();
    currentTool = "erase";
    document.getElementById("tool-erase").classList.add("border-white");
    document.getElementById("tool-brush").classList.remove("border-white");
  };
  els.flip.onclick = () => {
    playOkSound();
    isFlipped = !isFlipped;
    els.refineBase.style.transform =
      els.refineMask.style.transform = `scaleX(${isFlipped ? -1 : 1})`;
  };
  ...
  document.getElementById("confirm-refine-btn").onclick = async () => {
    ...
    const processedImage = processFinalStamp(
      els.refineBase,
      els.refineMask,
      els.ink.value,
      isFlipped,
    ...
    try {
      ...
      isFlipped = false;
      els.refineBase.style.transform = els.refineMask.style.transform =
        "scaleX(1)";
  ...
  document.getElementById("close-stamp-btn").onclick = () => {
    playReturnSound();
    els.addCont.classList.add("translate-y-full", "pointer-events-none");
    stopCamera(els.video, els.place);
  };
  ...
  document.getElementById("close-stamp-modal").onclick = () => {
    playReturnSound();
