...
export function populatePrefectures() {
  ...
  let html = `<div class="pref-option flex items-center px-6 py-4 cursor-pointer hover:bg-gray-50 border-b-[2px] border-black last:border-b-0" data-value="">
                    <span class="text-base font-black ">${escapeHtml(allPrefs)}</span>
                </div>`;
  state.prefectures.forEach((p) => {
    const id = p.pref_id || p.id;
    const name =
      lang === "ja"
        ? p.pref_name_jp || p.name_jp || p.pref_name_en || p.name_en
        : p.pref_name_en || p.name_en;
    html += `<div class="pref-option flex items-center px-6 py-4 cursor-pointer hover:bg-gray-50 border-b-[2px] border-black last:border-b-0" data-value="${id}" data-name="${escapeHtml(name)}">
                    <span class="text-base font-black ">${escapeHtml(name)}</span>
                </div>`;
  });
...
export function populateCompanies(filteredCompanies = state.companies) {
  ...
  let html = `<div class="comp-option flex items-center gap-4 px-6 py-4 cursor-pointer hover:bg-gray-50 border-b-[2px] border-black last:border-b-0" data-value="">
                    ...
                    <span class="text-base font-black ">${escapeHtml(allComps)}</span>
                </div>`;
  filteredCompanies.forEach((c) => {
    const id = c.company_id || c.id;
    const name =
      lang === "ja"
        ? c.company_name_jp || c.name_jp || c.company_name_en || c.name_en
        : c.company_name_en || c.name_en || c.company_name_jp;
...
export function updateCompanyDropdown() {
  ...
  const validLineIds = new Set(
    state.localStations
      .filter(
        (s) => String(s.pref_cd || s.pref_id) === String(state.currentPrefId),
      )
  ...
  populateCompanies(
    state.companies.filter((c) =>
      validCompanyIds.has(String(c.company_id || c.id)),
    ),
  );
}
...
export function handleSearch(e) {
  ...
  if (matchedLines.length === 0 && stationEntries.length === 0) {
    selectors.searchDropdown.innerHTML = `<div class="px-6 py-4 text-sm font-black  text-gray-400">${t("common.noResults")}</div>`;
  ...
  matchedLines.forEach(([id, data]) => {
    const lineName =
      lang === "ja"
        ? data.name_jp || data.name_en
        : data.name_en || data.name_jp;
    html += `<div class="search-result flex items-center gap-4 px-6 py-4 cursor-pointer hover:bg-gray-50 border-b-[2px] border-black last:border-b-0" data-type="line" data-line-id="${escapeHtml(id)}">
                    ...
                    <span class="text-sm font-black ">${escapeHtml(lineName)}</span>
                </div>`;
  });
  ...
  stationEntries.forEach(([, group]) => {
    if (group.stations.length === 1) {
      ...
      html += `<div class="search-result flex items-center gap-3 px-6 py-4 cursor-pointer hover:bg-gray-50 border-b-[2px] border-black last:border-b-0" data-type="station" data-line-id="${escapeHtml(s.line_id)}" data-station-id="${escapeHtml(s.id || s.station_id || "")}">
                        ...
                        <span class="text-sm font-black ">${escapeHtml(group.name)}</span>
                        <span class="ml-auto text-[10px] font-black  text-gray-400">${escapeHtml(lineName)}</span>
                    </div>`;
      return;
    }
    ...
    const subItems = group.stations
      .map((s) => {
        ...
        return `<div class="search-result flex items-center gap-2 px-6 py-3 cursor-pointer hover:bg-gray-50" data-type="station" data-line-id="${escapeHtml(s.line_id)}" data-station-id="${escapeHtml(s.id || s.station_id || "")}">
                        ...
                        <span class="text-xs font-black ">${escapeHtml(lineName)}</span>
                    </div>`;
      })
      .join("");
  ...
  selectors.searchDropdown
    ...
    .forEach((item) => {
      item.onclick = async () => {
        ...
        const stationNodes =
          selectors.detailStationsList?.querySelectorAll(
            ".station-name-click",
          ) || [];
        const stationNode = Array.from(stationNodes).find(
          (node) => String(node.dataset.stationId) === String(stationId),
        );
  ...
  selectors.searchDropdown
    .querySelectorAll(".multi-station-header")
    .forEach((header) => {
      header.onclick = () => {
        playSlideSound();
