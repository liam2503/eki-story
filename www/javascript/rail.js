...
export function showTooltip(latLng, data, type) {
    ...
    if (type === 'station') {
        ...
        if (statusContainer) {
            if (visited) {
                statusContainer.innerHTML = `
                    <div class="flex items-center justify-center gap-2">
                        <span data-i18n="map.visited" class="bg-[#B2FF59] border-[3px] border-black px-6 py-2 rounded-2xl text-xs font-black uppercase text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">Visited</span>
            ...
            // Tooltip content is injected dynamically, so re-apply i18n keys here.
            applyTranslations();
        }
