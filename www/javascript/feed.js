...
function setFeedFilter(filter) {
  ...
  if (filter === "all") {
    btnAll.className =
      "px-3 py-1 text-[10px] font-black  tracking-tighter rounded-lg bg-white dark:bg-slate-800 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-black dark:text-white transition-all cursor-pointer";
    btnFriends.className =
      "px-3 py-1 text-[10px] font-black  tracking-tighter rounded-lg text-gray-500 hover:text-black dark:hover:text-white transition-all cursor-pointer";
  } else {
    btnFriends.className =
      "px-3 py-1 text-[10px] font-black  tracking-tighter rounded-lg bg-white dark:bg-slate-800 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-black dark:text-white transition-all cursor-pointer";
    btnAll.className =
...
async function loadFeed(filter, reset = false) {
  ...
  try {
    ...
    if (filter === "all") {
      ...
      const friendIds = [...currentUserFriends, CURRENT_USER_ID].filter(
        Boolean,
      );
      const allDocs = [];
      ...
      for (let i = 0; i < friendIds.length; i += 30) {
        ...
        if (lastTimestampFriends !== null) {
          q = query(
            ...
            orderBy("timestamp", "desc"),
            where("timestamp", "<", lastTimestampFriends),
            limit(50),
          );
        } else {
          q = query(
      ...
      if (docs.length > 0)
        lastTimestampFriends = docs[docs.length - 1].data().timestamp;
    }

    if (docs.length === 0 && reset) {
      list.innerHTML = `<div class="text-center text-gray-400 font-bold text-sm  mt-10 tracking-widest">${t("feed.noPosts")}</div>`;
    } else {
...
function handleFeedClick(e) {
  ...
  if (stationTagBtn) {
    ...
    if (stationId && window.allStations) {
      const targetStation = window.allStations.find(
        (s) =>
      ...
      if (targetStation) {
        const panToTargetStation = () => {
          ...
          window.map.panTo({
            lat: Number(targetStation.lat),
            lng: Number(targetStation.lon),
          });
        };
        ...
        if (window.map) {
          ...
          window.addEventListener("mapInitialized", panToTargetStation, {
            once: true,
          });
        }
      }
...
function createPostElement(id, data, isDetail = false) {
  ...
  const deleteBtnHtml = isOwner
    ? `<button class="delete-post-btn ${btnSize} bg-[#FF5252] border-[3px] border-black dark:border-slate-600 rounded-full flex items-center justify-center text-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all" data-id="${id}"><svg class="${svgSize}" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>`
    : "";
  ...
  if (data.tag) {
    ...
    tagsHtml.push(
      `<span class="bg-[#FF80AB] border-[3px] border-black dark:border-slate-600 px-2 py-1 rounded-lg text-[9px] font-black  tracking-tighter text-black">${escapeHtml(tagLabel)}</span>`,
    );
  }
  if (data.stationId || data.stationName) {
    ...
    if (data.stationId && window.allStations) {
      ...
      if (s) {
        stationLabel =
          lang === "ja"
            ? s.station_name_jp || s.station_name_en
    ...
    if (stationLabel) {
      if (data.stationId) {
        tagsHtml.push(
          `<button class="station-tag-btn relative z-10 bg-[#40C4FF] border-[3px] border-black dark:border-slate-600 px-2 py-1 rounded-lg text-[9px] font-black  tracking-tighter text-black hover:-translate-y-0.5 hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:translate-y-0 active:shadow-none transition-all cursor-pointer" data-id="${data.stationId}">${escapeHtml(stationLabel)}</button>`,
        );
      } else {
        tagsHtml.push(
          `<span class="bg-[#40C4FF] border-[3px] border-black dark:border-slate-600 px-2 py-1 rounded-lg text-[9px] font-black  tracking-tighter text-black">${escapeHtml(stationLabel)}</span>`,
        );
      }
    }
  }
  const tagContainer = tagsHtml.length
    ? `<div class="flex flex-wrap gap-1.5 mt-3">${tagsHtml.join("")}</div>`
    : "";
  ...
  div.innerHTML = `
        <div class="flex items-center justify-between mb-3">
            <div class="flex flex-col">
                <h3 class="font-black ${titleSize}  tracking-tighter dark:text-white">${escapeHtml(data.username)}</h3>
                <span class="text-[8px] font-black text-gray-400  tracking-widest">${new Date(data.timestamp).toLocaleString()}</span>
            </div>
        ...
        <div class="flex gap-3 mt-4">
            <button class="yeah-btn flex-1 ${yeahColor} border-[3px] border-black dark:border-slate-600 rounded-lg py-1.5 ${yeahText} font-black text-xs  tracking-tighter shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-y-[3px] active:translate-x-[3px] active:shadow-none transition-all flex items-center justify-center gap-1.5" data-id="${id}">
            ...
            <button class="${isDetail ? "opacity-50 pointer-events-none" : "talk-btn post-detail-trigger"} flex-1 bg-white dark:bg-slate-700 border-[3px] border-black dark:border-slate-600 rounded-lg py-1.5 text-black dark:text-white font-black text-xs  tracking-tighter shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-y-[3px] active:translate-x-[3px] active:shadow-none transition-all flex items-center justify-center gap-1.5" data-id="${id}">
...
function initFeedStationSearch() {
  ...
  searchInput.addEventListener("input", function () {
    ...
    if (entries.length === 0) {
      searchResults.innerHTML = `<div class="px-5 py-4 text-xs font-black  text-gray-400">${t("common.noResults")}</div>`;
    ...
    entries.forEach(([, group]) => {
      ...
      const colorDots = group.stations
        .slice(0, 4)
        .map((s) => {
      ...
      html += `
                <div class="feed-station-result flex items-center gap-3 px-5 py-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700 border-b-[2px] border-black dark:border-slate-600 last:border-b-0"
                    ...
                    <span class="text-xs font-black  dark:text-white">${escapeHtml(name)}</span>
                </div>
            `;
    });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (closePostBtn) {
    closePostBtn.onclick = () => {
      playReturnSound();
      document
        .getElementById("create-post-container")
        .classList.add("translate-y-full", "pointer-events-none");
      stopCamera(
        document.getElementById("feed-camera-feed"),
        document.getElementById("feed-camera-placeholder"),
      );
    };
  }
  ...
  if (submitBtn) {
    submitBtn.onclick = async () => {
      ...
      const caption = document
        .getElementById("post-caption-input")
        .value.trim();
      ...
      if (stationId && window.allStations) {
        const lang = getLanguage();
        const s = window.allStations.find(
          (x) => String(x.id) === String(stationId),
        );
        if (s) {
          stationName =
            lang === "ja"
      ...
      try {
        ...
        if (pendingPostImage) {
          const imageRef = ref(
            storage,
            `posts/${CURRENT_USER_ID}_${Date.now()}.jpg`,
          );
  ...
  if (closeDetailBtn) {
    closeDetailBtn.onclick = () => {
      playReturnSound();
      document
        .getElementById("post-detail-container")
...
function renderDetailPost() {
  ...
  content.innerHTML = "";
  content.appendChild(
    createPostElement(currentDetailPostId, currentDetailPostData, true),
  );
  ...
  if (yeahBtn) {
    yeahBtn.onclick = () =>
      toggleYeah(
        currentDetailPostId,
        yeahBtn.classList.contains("bg-[#FF80AB]"),
      );
  }
...
function openPostDetail(id) {
  ...
  commentsUnsubscribe = onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach((docSnap) => {
      ...
      item.className =
        "bg-gray-100 dark:bg-slate-800 border-[3px] border-black dark:border-slate-600 p-4 rounded-2xl flex flex-col gap-1 relative";
      const delHtml = isOwner
        ? `<button class="absolute top-3 right-3 text-gray-400 hover:text-black dark:hover:text-white transition-colors" onclick="window.deleteComment('${id}', '${docSnap.id}')"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>`
        : "";
      item.innerHTML = `
                <div class="flex items-center gap-2">
                    <span class="font-black text-sm  dark:text-white">${escapeHtml(data.username)}</span>
    ...
    setTimeout(() => {
      cont.querySelector(".flex-1").scrollTop =
        cont.querySelector(".flex-1").scrollHeight;
    }, 50);
  });
}
